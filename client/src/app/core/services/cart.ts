...
export class CartService {
  ...
  selectDelivery = signal<DeliveryMethod | null>(null);
  total = computed(() =>{
  ...
  setCart(cart: CartM){
    return this.http.post<CartM>(this.baseUrl + 'cart', cart).pipe(
      tap(cart => {
        this.cart.set(cart)
      })
    )
  ...
  async addItemToCart(item: CartItemM | Product, quantity = 1){
